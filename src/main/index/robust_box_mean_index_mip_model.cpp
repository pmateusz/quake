#include "robust_box_mean_index_mip_model.h"

quake::RobustBoxMeanIndexMipModel::RobustBoxMeanIndexMipModel(const quake::ExtendedProblem *problem,
...
void quake::RobustBoxMeanIndexMipModel::Build(const boost::optional<Solution> &solution) {
            ...
            = util::CreateVarMatrix(mip_model_, NumStations(), NumCloudCoverPeriods(), -GRB_INFINITY, GRB_INFINITY, "mean_cc");
    {
        ...
        for (const auto &station : ObservableStations()) {
            ...
            for (const auto &cloud_cover_period : ObservableCloudCover(station)) {
                ...
                mean_upper_bound_expr += mean_value * mean_dual_var;
        ...
        mip_model_.addConstr(mean_upper_bound_expr <= 0);
    }

    {
        ...
        GRBLinExpr intercept_lower_bound_expr = -riskiness_index;
        for (const auto &station : ObservableStations()) {
            const auto station_index = Index(station);
            for (const auto &cloud_cover_period : ObservableCloudCover(station)) {
                const auto cloud_cover_index = CloudCoverIndex(cloud_cover_period);
        ...
        GRBLinExpr objective = riskiness_index;
