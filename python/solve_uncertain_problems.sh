#!/usr/bin/env bash
SOLVER_DET=/home/pmateusz/dev/quake/cmake-build-debug/quake-det
PROBLEM_DIR=/home/pmateusz/dev/quake/current_review/problems
...
for ((index=0; index<${#PROBLEMS[@]}; ++index)); do
    $SOLVER_DET --problems="${PROBLEMS[index]}" --gap-limit=0.05 --interval-step=00:01:00 --output=solution_det_$index.json
done